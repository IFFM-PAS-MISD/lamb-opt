...
\  \DIFdel{emph} \DIFadd{textbf} { Bold }
...
curves of Lamb waves.
	...
	(along fibres), Poisson's ratio of the matrix $\nu_m$, Poisson's ratio of the fibres 
	$\nu_f$ and volume fraction of reinforcing fibres $V$.  The fibre density was 
	calculated based on the rule of mixture as:
	\begin{equation}
	\rho_f = \frac{\rho - \rho_m (1-V)}{V_1},
	\end{equation}
	where \DIFadd{ $V$} is the volume fraction of reinforcing fibres. Hence, only 6~variables needs 
	to be determined.
	...
	and $C_{66}$. Hence, 9 variables needs to be determined.
\end{enumerate}
...
LaTeX margins are related to the document's paper size. To format a document for A4 
...
in Refs.~\citenum{Alred03}, for example.  Many aspects of 
technical writing are addressed in the {\em AIP Style Manual}, published by the 
American Institute of Physics  \citenum{Lees-Miller-LaTeX-course-1}.  It is available on 
...
Tables are handled identically to figures, except that their captions appear above the table. 
   ...
   \includegraphics{E:/work/projects/opus15/lamb-opt/reports/figures/SASE/SASE20_plain_weave_out/SASE20_plain_weave_angle_0_param_dispersion_curves_color.png}
   \end{tabular}
   \end{center}
   \caption[example] 
%>>>> use \label inside caption to get Fig. number with \ref{}
   { \label{fig:C11} 
Influence of $C_{11}$ elastic constant on the dispersion curves.}
   \end{figure} 
...
importance; e.g., \(\beta = b/r\).  Important equations appear on their own line.  Such 
