...
modelname = 'ga_pzt_unidirectional_A0L_A0H_S0_A1_A2_SH0';
radians = false;
...
% create path to optimized dispersion curves
...
for j=1:number_of_angles % beta
    ...
    if(overwrite||(~overwrite && ~exist([output_path,figfilename,'.png'], 'file')))
        ...
        plot(fvec2(2:end)/1e3,kvec(2:end),'m','linewidth',LW);
        plot(fvec3(2:end)/1e3,kvec(2:end),'c','linewidth',LW);
