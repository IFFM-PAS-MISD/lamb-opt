...
% and the results are stored in numerical processed data folder
...
for k = 1:nFile
    ...
    if(overwrite||(~overwrite && ~exist([processed_data_path,processed_filename], 'file')))
        try 
            ...
            %% PROCESS DATA
            fprintf('Processing:\n%s\n',filename);
            [Data_polar,number_of_wavenumber_points,wavenumber_max] = cartesian_to_polar_wavenumber_wavefield(KXKYF_,kxmax,kymax,beta); 
