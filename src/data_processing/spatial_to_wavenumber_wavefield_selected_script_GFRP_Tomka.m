...
for k = test_case
    filename = list{k};
    processed_filename = ['interim_',filename,specimen_name,'_KXKYF']; % filename of processed .mat data
    % check if already exist
    if(overwrite||(~overwrite && ~exist([interim_data_path,processed_filename,'.mat'], 'file')))
        try 
            % load raw experimental data file
            disp('loading data');
            load([raw_data_path,filename]); % Data WL (Length Width) time
            %% PROCESS DATA
            fprintf('Processing:\n%s\n',filename);
            [KXKYF_,kx_vec,ky_vec,f_vec] = spatial_to_wavenumber_wavefield(Data,WL(1),WL(2),time);
            %% END OF PROCESSING
