...
function [K, M] = get_KM(nele_layer,np,h,C,rho)
...
% 1D problem: mesh only in stacking (X) direction
% DoF at each node = 3
nD = 1;                                     % do NOT change
...
xcoord_IEN = zeros(nnodes_ele,nele);
...
%         ele_length = xcoord_IEN(nnodes_ele,ele_no) - xcoord_IEN(1,ele_no);
        ...
        % Assemle element stiffness and mass matrices
